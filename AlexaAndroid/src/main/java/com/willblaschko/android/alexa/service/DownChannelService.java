...
public class DownChannelService extends Service {
    ...
    public static final String ACTION_CHANNEL_STATUS = "DownChannelServiceStatus";

    private static final String TAG = "DownChannelService";

    private final IBinder mBinder = new DownChannelBinder();

    private AlexaManager alexaManager;
    private Call currentCall;
    private AndroidSystemHandler handler;
    private DownChannelStatusListener mStatusListener;
                ...
                currentCall.enqueue(new Callback() {
                    ...
                    public void onFailure(Call call, IOException e) {
                        ...
                        if (mStatusListener != null) {
                            mStatusListener.onStatusChanged(DISCONNECTED);
                        }
                        Intent status = new Intent(ACTION_CHANNEL_STATUS);
                        status.putExtra("status", DISCONNECTED);
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ...
                        if (mStatusListener != null) {
                            mStatusListener.onStatusChanged(CONNECTED);
                        }

                        Log.d(TAG, "Down Channel request status " + response.code());
                        ...
                        alexaManager.sendEvent(Event.getSynchronizeStateEvent(), new ImplAsyncCallback<AvsResponse, Exception>() {
                            ...
                            public void success(AvsResponse result) {
                                ...
                                if (result.isEmpty()) {
                                    Intent status = new Intent(ACTION_CHANNEL_STATUS);
                                    status.putExtra("status", CONNECTED);
                                    sendBroadcast(status);
                                }
                            }
    ...
    public class DownChannelBinder extends Binder {
        ...
        public void setDownChannelStatusListener(DownChannelStatusListener listener) {
            mStatusListener = listener;
        }

    }

    public interface DownChannelStatusListener {
