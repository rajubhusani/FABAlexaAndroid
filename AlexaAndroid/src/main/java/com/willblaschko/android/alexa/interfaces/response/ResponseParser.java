    ...
    public static AvsItem parseDirective(Directive directive, HashMap<String, ByteArrayInputStream> audio) throws IOException {
        ...
        switch (directive.getHeader().getName()) {
            ...
            case Directive.TYPE_RENDER_TEMPLATE:
                return new AvsRenderTemplateItem(directive.getPayload().getToken(),
                        directive.getPayload().getType(),
                        directive.getPayload().getTitle(),
                        directive.getPayload().getSkillIcon(),
                        directive.getPayload().getTextField(),
                        directive.getPayload().getImage());
            default:
