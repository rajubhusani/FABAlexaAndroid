...
public class AuthorizationManager {
    ...
    public void logoutUser(AuthorizationCallback callback) {
        mCallback = callback;

        com.amazon.identity.auth.device.api.authorization.AuthorizationManager.signOut(mContext, new Listener<Void, AuthError>() {
            @Override
            public void onSuccess(Void aVoid) {
                TokenManager.clearTokens(mContext);
                mCallback.onSuccess();
            }

            @Override
            public void onError(AuthError authError) {
                mCallback.onError(authError);
            }
        });
    }
